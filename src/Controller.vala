    ...
    public class Controller : GLib.Object {
        ...
        public void play() {
            ...
            if(current_episode != null) {
                ...
                if(current_episode.last_played_position > 0 && current_episode.last_played_position > player.progress) {
                    ...
                    if(current_episode.download_status == DownloadStatus.NOT_DOWNLOADED) {
                        player.pause();
                    }
        ...
        */
        ...
        public void add_podcast_feed(string feed) {
            ...
            string entered_feed = itunes.get_rss_from_itunes_url(feed);
            if(entered_feed == null) {
                entered_feed = feed;
            }
            ...
            if(success) {
                ...
                var add_err_dialog = new Gtk.MessageDialog(window, Gtk.DialogFlags.MODAL, Gtk.MessageType.ERROR, Gtk.ButtonsType.OK, "");
                add_err_dialog.response.connect((response_id) => {
                    info("response from add_err_dialog_ignored");
